...
class TelegramRequestHandler
{
    public function handle(Request $request) {
        try {
            ...
            if($text == '/start') {
                ...
                $this->processMessage($message, $author, $chat_id);
    ...
    public function processMessage(Message $message, Author $author, $chat_id = null) {
        $bot = $message->telegram_bot;
        ...
        try {
            ...
            if ($message->message_files->count()) {
                ...
                if($chat_id) $botApi->sendMediaGroup($chat_id, $mediaArr['media'], null, null, null, null, null, $mediaArr['attachments']);
                $botApi->sendMediaGroup($message->telegram_bot->moderation_group, $mediaArr['media'], null, null, null, null, null, $mediaArr['attachments']);
            } else {
                if($chat_id) $botApi->sendMessage($chat_id, $message->text, 'HTML', true);
            ...
            if($chat_id) $botApi->sendMessage($chat_id, __('webapp.message_accepted') . " #" . $message->id, 'HTML', true);
