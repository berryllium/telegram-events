...
@section('content')
    <form method="post" action="{{ route('place.update', [$place]) }}" enctype="multipart/form-data">
        ...
        <div class="mb-2">
            ...
            <input id="is_channel" type="checkbox" class="form-check-input" name="is_channel" value="1" {{ $place->is_channel ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="service_title" name="service_title" value="{{ $place->service_title }}">
            @error('service_title')
            <div class="form-text text-danger">{{ $message }}</div>
            @enderror
        </div>
